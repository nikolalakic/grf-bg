...
if nied > 0.6:
    ...
    if nied <= 0.2 and q0 <= 2:
        print('Primenjuju se pravila iz EC2!')
    if nied > 0.2:
        ...
        n = int(input('Unesi broj pridrzanih sipki [kom]: '))
        bi = float(input('Unesi razmak pridrzanih sipki bi [cm]: '))
        ...
        if a == 1 and T >= Tc:
            ...
            mfi = 1 + 2 * (q0 - 1) * Tc / T
        alfan = (1 - n*math.pow(bi, 2)/(6*b0*h0))
        for x in finiz:
            Vsw = x*ObimUzg
            for p in sniz:
